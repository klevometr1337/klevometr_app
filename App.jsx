...
// ── Sound Engine (Advanced Web Audio API) ──
let audioCtx = null;
...
// ── ЗАБРОС: свист удилища + шелест лески ──
function playCastSound() {
  try {
    ...
    // 1. Rod swoosh — filtered noise sweep
    const noise = ctx.createBufferSource();
    noise.buffer = createNoise(ctx, 0.25);
    const swooshFilter = ctx.createBiquadFilter();
    swooshFilter.type = "bandpass";
    swooshFilter.frequency.setValueAtTime(3000, t);
    swooshFilter.frequency.exponentialRampToValueAtTime(400, t + 0.2);
    swooshFilter.Q.setValueAtTime(2, t);
    ...
    // 2. Line whizz — descending tone
    const line = ctx.createOscillator();
    line.type = "sawtooth";
    line.frequency.setValueAtTime(1200, t + 0.02);
    line.frequency.exponentialRampToValueAtTime(150, t + 0.18);
    const lineFilter = ctx.createBiquadFilter();
    lineFilter.type = "lowpass";
    lineFilter.frequency.setValueAtTime(2000, t);
    lineFilter.frequency.exponentialRampToValueAtTime(300, t + 0.18);
    const lineGain = ctx.createGain();
    lineGain.gain.setValueAtTime(0.15, t + 0.02);
    lineGain.gain.exponentialRampToValueAtTime(0.01, t + 0.2);
    line.connect(lineFilter);
    lineFilter.connect(lineGain);
    lineGain.connect(master);
    line.start(t + 0.02);
    line.stop(t + 0.22);

    // 3. Subtle impact tick
    const tick = ctx.createOscillator();
    tick.type = "triangle";
    tick.frequency.setValueAtTime(800, t + 0.16);
    tick.frequency.exponentialRampToValueAtTime(200, t + 0.22);
    const tickGain = ctx.createGain();
    tickGain.gain.setValueAtTime(0.12, t + 0.16);
    tickGain.gain.exponentialRampToValueAtTime(0.01, t + 0.25);
...
// ── ПОКЛЁВКА: реалистичный рыболовный колокольчик ──
function playBiteSound() {
  try {
    ...
    // Bell — additive synthesis with inharmonic partials (real bells are not harmonic)
    const bellPartials = [
      { freq: 845, amp: 1.0, decay: 1.2 },    // fundamental (slightly detuned)
      { freq: 1688, amp: 0.7, decay: 0.9 },    // ~2nd partial
      { freq: 2510, amp: 0.4, decay: 0.7 },    // ~3rd (inharmonic!)
      { freq: 3365, amp: 0.25, decay: 0.5 },   // ~4th
      { freq: 4200, amp: 0.15, decay: 0.35 },  // 5th
      { freq: 5050, amp: 0.08, decay: 0.25 },  // shimmer
    ];
    ...
    [0, 0.18].forEach((offset, strike) => {
      bellPartials.forEach(p => {
        ...
        osc.type = "sine";
        osc.frequency.setValueAtTime(p.freq * (strike === 1 ? 1.003 : 1), t + offset); // slight detune on 2nd
        const gain = ctx.createGain();
        const vol = p.amp * (strike === 0 ? 0.3 : 0.25);
        gain.gain.setValueAtTime(0, t + offset);
        gain.gain.linearRampToValueAtTime(vol, t + offset + 0.003); // sharp attack
        ...
        gain.connect(master);
        osc.start(t + offset);
        osc.stop(t + offset + p.decay + 0.05);
      });

      // Impact transient — click of the bell clapper
      const click = ctx.createBufferSource();
      click.buffer = createNoise(ctx, 0.015);
      const clickFilter = ctx.createBiquadFilter();
      clickFilter.type = "highpass";
      clickFilter.frequency.setValueAtTime(4000, t + offset);
      const clickGain = ctx.createGain();
      clickGain.gain.setValueAtTime(0.15, t + offset);
      clickGain.gain.exponentialRampToValueAtTime(0.001, t + offset + 0.012);
      click.connect(clickFilter);
      clickFilter.connect(clickGain);
      clickGain.connect(master);
      click.start(t + offset);
      click.stop(t + offset + 0.015);
    });
  } catch(e) {}
}

// ── ПОЙМАЛ: сплеш воды + победный аккорд ──
function playCaughtSound() {
  try {
    ...
    // 1. Water splash — filtered noise burst
    const splash = ctx.createBufferSource();
    splash.buffer = createNoise(ctx, 0.3);
    const splashFilter = ctx.createBiquadFilter();
    splashFilter.type = "bandpass";
    splashFilter.frequency.setValueAtTime(1500, t);
    splashFilter.frequency.exponentialRampToValueAtTime(300, t + 0.25);
    splashFilter.Q.setValueAtTime(0.8, t);
    const splashGain = ctx.createGain();
    splashGain.gain.setValueAtTime(0.4, t);
    splashGain.gain.linearRampToValueAtTime(0.5, t + 0.03);
    splashGain.gain.exponentialRampToValueAtTime(0.01, t + 0.28);
    ...
    // 2. Bubble pops — short filtered clicks
    [0.05, 0.1, 0.15, 0.22].forEach(offset => {
      const pop = ctx.createOscillator();
      pop.type = "sine";
      pop.frequency.setValueAtTime(300 + Math.random() * 400, t + offset);
      pop.frequency.exponentialRampToValueAtTime(80, t + offset + 0.04);
      const popGain = ctx.createGain();
      popGain.gain.setValueAtTime(0.12, t + offset);
      popGain.gain.exponentialRampToValueAtTime(0.001, t + offset + 0.05);
      pop.connect(popGain);
      popGain.connect(master);
      pop.start(t + offset);
      pop.stop(t + offset + 0.06);
    });

    // 3. Victory chord (after splash) — rich major chord C-E-G-C with harmonics
    const chordNotes = [
      { freq: 523.25, type: "triangle", amp: 0.22 },  // C5
      { freq: 659.25, type: "sine", amp: 0.18 },       // E5
      { freq: 783.99, type: "sine", amp: 0.16 },       // G5
      { freq: 1046.5, type: "triangle", amp: 0.2 },    // C6
    ];
    const chordStart = t + 0.2;
    chordNotes.forEach((note, i) => {
      const osc = ctx.createOscillator();
      osc.type = note.type;
      osc.frequency.setValueAtTime(note.freq, chordStart + i * 0.08);
      // Slight vibrato for richness
      const vibrato = ctx.createOscillator();
      vibrato.frequency.setValueAtTime(5, chordStart);
      const vibratoGain = ctx.createGain();
      vibratoGain.gain.setValueAtTime(2, chordStart);
      vibrato.connect(vibratoGain);
      vibratoGain.connect(osc.frequency);
      vibrato.start(chordStart);
      vibrato.stop(chordStart + 1.2);

      const gain = ctx.createGain();
      gain.gain.setValueAtTime(0, chordStart + i * 0.08);
      gain.gain.linearRampToValueAtTime(note.amp, chordStart + i * 0.08 + 0.04);
      gain.gain.setValueAtTime(note.amp * 0.9, chordStart + i * 0.08 + 0.3);
      gain.gain.exponentialRampToValueAtTime(0.001, chordStart + 1.2);
      osc.connect(gain);
      gain.connect(master);
      osc.start(chordStart + i * 0.08);
      osc.stop(chordStart + 1.3);
    });

    // 4. Sparkle — high shimmer on top
    [0.35, 0.5, 0.65].forEach((offset, i) => {
      const sparkle = ctx.createOscillator();
      sparkle.type = "sine";
      sparkle.frequency.setValueAtTime(2093 + i * 400, t + offset);
      const sGain = ctx.createGain();
      sGain.gain.setValueAtTime(0, t + offset);
      sGain.gain.linearRampToValueAtTime(0.06, t + offset + 0.01);
      sGain.gain.exponentialRampToValueAtTime(0.001, t + offset + 0.15);
      sparkle.connect(sGain);
      sGain.connect(master);
      sparkle.start(t + offset);
      sparkle.stop(t + offset + 0.2);
    });
  } catch(e) {}
}
